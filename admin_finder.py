...
#If You Want To Delete This Script, You Can Tell Me In My Telegram account
# https://t.me/DaxxzMods
...
slowprint("Thanks To Using This Script!!")
clear_terminal()
welcome_message = Fore.BLUE + '''
██╗    ██╗███████╗██╗      ██████╗ ██████╗ ███╗   ███╗███████╗██╗██╗
██║    ██║██╔════╝██║     ██╔════╝██╔═══██╗████╗ ████║██╔════╝██║██║
██║ █╗ ██║█████╗  ██║     ██║     ██║   ██║██╔████╔██║█████╗  ██║██║
██║███╗██║██╔══╝  ██║     ██║     ██║   ██║██║╚██╔╝██║██╔══╝  ╚═╝╚═╝
...
|———————————————————————————————————————|
| Creator: ./DaxxzXploit                |
| Telegram: https://t.me/DaxxzMods      |
| Instagram: daxxz.xploit               |
| Github: DaxxzXploit                   |
...
logo = '''
  ____              _          _____           _     
 / ___| _ __   __ _| | _____  |_   _|__   ___ | |___ 
 \___ \| '_ \ / _` | |/ / _ \   | |/ _ \ / _ \| / __|
  ___) | | | | (_| |   <  __/   | | (_) | (_) | \__ \
 |____/|_| |_|\__,_|_|\_\___|   |_|\___/ \___/|_|___/
                  By ./DaxxzXploit
'''
...
def cek_halaman(base_url, wordlist):
    """
    Memeriksa setiap halaman berdasarkan wordlist.
    
    Args:
        base_url (str): URL dasar dari website (contoh: https://example.com/).
        wordlist (list): Daftar kata yang digunakan untuk pengecekan halaman.
    """
    for endpoint in wordlist:
        ...
        try:
            ...
            if response.status_code == 200:
                print(Fore.GREEN + f"[!] Halaman ditemukan: {url}")
            elif response.status_code == 403:
                print(Fore.RED + f"[!] Akses dilarang: {url}")
            elif response.status_code == 404:
                print(Fore.RED + f"[!] Halaman tidak ditemukan: {url}")
            else:
                print(f"[{response.status_code}] Status lain: {url}")
        except requests.exceptions.RequestException as e:
            print(Fore.RED + f"[!] Kesalahan saat mengakses {url}: {e}")

# Fungsi utama
def main():
    # URL dasar website yang ingin diperiksa
    base_url = input("Contoh Penggunaan:\nhttps://www.example.com\nMasukan Url:")

    # Daftar kata yang akan diperiksa (bisa berasal dari file wordlist.txt)
    wordlist = [
...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf','cpanel','cpanel.php','cpanel.html'
    ...
    # Memanggil fungsi untuk memeriksa halaman
    cek_halaman(base_url, wordlist)
